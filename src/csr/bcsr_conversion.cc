...
BCSR::BCSR(BLIL matrix) : _width(matrix._width), _height(matrix._height) {
    size_t nnz = 0;
    for (size_t i = 0; i < _height; i++) { nnz += matrix._rows[i].size(); }
    _indices = std::vector<uint32_t>(nnz);
    _index_pointers = std::vector<uint32_t>(_height + 1, 0);
    nnz = 0;
    for (size_t r = 0; r < _height; r++) {
        _index_pointers[r] = nnz;
        for (size_t c = 0; c < matrix._rows[r].size(); c++) { _indices[nnz++] = matrix._rows[r][c]; }
    }
...
std::vector<uint8_t> BCSR::toDenseMatrix() const {
    std::vector<uint8_t> mat(_width * _height);

    for (int i = 0; i < _height; ++i) {
        for (int idx = _index_pointers[i]; idx < _index_pointers[i + 1]; ++idx) { mat[i * _width + _indices[idx]] = 1; }
    }
...
std::string BCSR::toString() const {
    ...
    for (uint32_t r = 0; r < _height; r++) {
        uint32_t ones = _index_pointers[r + 1] - _index_pointers[r];
        nbPerLine[r] = ones;
    ...
    oss << "<" << _height << ";" << _width << "> (" << _index_pointers[_height] << " ones / " << (_width * _height)
        << ", sparsity: " << float(100.0 * (_width * _height - _index_pointers[_height]) / (_width * _height))
        << "%, ones per line: min=" << minNbPerLine << ", med=" << medNbPerLine << ", max=" << maxNbPerLine << ")\nIndex Pointers (Rows): [";
...
std::string BCSR::toCondensedString(char const separator) const {
    if (_width == 0 || _height == 0) return "(1)[0]\n(0)[]";
...
void BCSR::insertDn2BCSR(uint8_t values[]) {
    _index_pointers[0] = 0;
