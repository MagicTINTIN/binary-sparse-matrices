...
BLIL::BLIL() : _height(0), _width(0) {
    _rows = std::vector<std::vector<uint32_t>>(0);
}

BLIL::BLIL(uint32_t height, uint32_t width) : _height(height), _width(width) {
    _rows = std::vector<std::vector<uint32_t>>(_height);
}

BLIL::BLIL(uint32_t height, uint32_t width, std::vector<std::vector<uint32_t>> &rows) : _height(height), _width(width) {
    _rows = rows;
}

BLIL::BLIL(uint32_t height, uint32_t width, uint8_t values[]) : _height(height), _width(width) {
    _rows = std::vector<std::vector<uint32_t>>(_height);
...
bool BLIL::checkOrder(bool verbose) const {
    if (_height != _rows.size()) {
        if (verbose) fprintf(stderr, "Height (%d) is not equal to number of rows (%ld)!\n", _height, _rows.size());
        return false;
    }

    size_t r = 0;
    for (std::vector<uint32_t> col : _rows) {
        if (col.size() > 0 && _width <= col[0]) {
            if (verbose) fprintf(stderr, "Width (%d) is not equal to column number (%d) in line %ld!\n", _width, col[0], r);
            return false;
        }
...
bool BLIL::operator==(const BLIL &b) {
    return _height == b._height && _width == b._width && _rows == b._rows;
}
...
void BLIL::operationOr2(const BLIL &b) {
    ...
    for (size_t r = 0; r < _height; r++) {
        ...
        for (; Bcol_idx < B_col_nb; Bcol_idx++) { _rows[r].emplace_back(b._rows[r][Bcol_idx]); }
    }
}
...
BLIL BLIL::operationTimesMatrix(const BLIL &b) const {
    ...
    for (uint32_t i = 0; i < _height; i++) {
        ...
        for (uint32_t j : _rows[i]) {

            for (uint32_t k : b._rows[j]) {
                if (next[k] == -1) {
                    next[k] = head;
                    head = k;
                    length++;
...
BLIL BLIL::transpose_withoutConversion() const {
    ...
    for (size_t r = 0; r < _height; r++)
        for (uint32_t c : _rows[r]) insertByValue<uint32_t>(res._rows[c], r);
    return res;
}
...
void BLIL::set(uint32_t const row, uint32_t const col, uint8_t const value) {
    if (value) set(row, col);
    else reset(row, col);
}

void BLIL::set(uint32_t const row, uint32_t const col) {
...
void BLIL::reset(uint32_t const row, uint32_t const col) {
    if (col >= _width || row >= _height) {
...
bool BLIL::get(uint32_t const row, uint32_t const col) const {
    if (col >= _width || row >= _height) {
...
void BLIL::addDimensionNZC(uint32_t nonzero_column) {
    addDimension();
    _rows[_height - 1].emplace_back(nonzero_column);
}

void BLIL::addDimensionNZR(uint32_t nonzero_row) {
    addDimension();
    _rows[nonzero_row].emplace_back(_width - 1);
}

void BLIL::addDimension(uint32_t nonzero_column, uint32_t nonzero_row) {
    addDimension();

    _rows[_height - 1].emplace_back(nonzero_column);
    if (nonzero_row < _height - 1) _rows[nonzero_row].emplace_back(_width - 1);
}

BLIL::stats BLIL::getStats() {
    if (_width == 0 || _height == 0) return stats{0, 0, 0, 0, 0, 0, 0};

    size_t nnz = 0;
    std::vector<uint32_t> nbPerLine(_height);
    uint32_t minNbPerLine(__INT32_MAX__), maxNbPerLine(0);
    for (size_t i = 0; i < _height; i++) {
        uint32_t ones = (uint32_t)_rows[i].size();
        nnz += ones;
        nbPerLine[i] = ones;
    ...
    uint32_t medNbPerLine = nbPerLine[_height / 2];

    return stats{
        _height, _width, minNbPerLine, medNbPerLine, maxNbPerLine, nnz, float(100.0 * (_width * _height - nnz) / (_width * _height))};
}

std::vector<uint32_t> BLIL::getRow(uint32_t row) const {
    return _rows[row];
}

std::vector<std::vector<uint32_t>> const &BLIL::getRows() const {
    return _rows;
}
