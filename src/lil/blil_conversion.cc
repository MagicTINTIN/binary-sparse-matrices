...
BLIL::BLIL(BCSR matrix) : _width(matrix._width), _height(matrix._height) {
    ...
    for (size_t r = 0; r < _height; r++) {
        uint32_t r_start = matrix._index_pointers[r];
        uint32_t r_stop = matrix._index_pointers[r + 1];
        std::vector<uint32_t> line(r_stop - r_start);
        for (uint32_t c = r_start, cc = 0; c < r_stop; c++, cc++) { line[cc] = matrix._indices[c]; }
        _rows[r] = line;
    }
}
...
void BLIL::insertDn2BLIL(uint8_t values[]) {
    for (uint32_t row = 0; row < _height; ++row) {
        for (uint32_t col = 0; col < _width; ++col) {
            if (values[row * _width + col]) { _rows[row].emplace_back(col); }
        }
    }
}

std::vector<uint8_t> BLIL::toDenseMatrix() const {
    std::vector<uint8_t> mat(_width * _height);

    for (int r = 0; r < _height; ++r) {
        for (uint32_t c : _rows[r]) mat[r * _width + c] = 1;
    }
...
std::string BLIL::toString() const {
    ...
    oss << "<" << _height << ";" << _width << "> (" << nnz << " ones / " << (_width * _height)
        << ", sparsity: " << float(100.0 * (_width * _height - nnz) / (_width * _height)) << "%, ones per line: min=" << minNbPerLine
...
std::string BLIL::info(bool excludeFullLineStat) const {
    ...
    for (size_t i = 0; i < _height; i++) {
        ...
        nbPerLine[i] = ones;
        if (ones < minNbPerLine) minNbPerLine = ones;
        if (ones > maxNbPerLine && !(excludeFullLineStat && ones == _width)) maxNbPerLine = ones;
    }
    std::sort(nbPerLine.begin(), nbPerLine.end());
    uint32_t medNbPerLine = nbPerLine[_height / 2];
...
std::string BLIL::toDnString() const {
    if (_height == 0) return std::string("[]");
    if (_width == 0) {
        std::string ret("[[]");
        for (size_t i = 1; i < _height; i++) { ret += ",[]"; }
        return ret + "]";
    }

    std::vector<uint8_t> m = toDenseMatrix();

    return denseMatrixPrinter(m, _height, _width);
}

std::string centerString(uint8_t width, std::string const &str) {
    uint8_t len = str.length();
    if (width < len) { return str; }

    uint8_t diff = width - len;
    uint8_t pad1 = diff / 2;
    uint8_t pad2 = diff - pad1;
    return std::string(pad1, ' ') + str + std::string(pad2, ' ');
}

void addNChars(std::ostringstream &oss, std::string const &str, uint32_t const times) {
    for (size_t i = 0; i < times; i++) { oss << str; }
}

std::string nTimesString(std::string const &str, uint32_t const times) {
    std::ostringstream oss;
    for (size_t i = 0; i < times; i++) { oss << str; }
    return oss.str();
}

void addNumber(std::ostringstream &oss, uint32_t const &number, uint8_t const size) {
    oss << centerString(size, std::to_string(number));
}

void addRightNumber(std::ostringstream &oss, uint32_t const &number, uint8_t const size) {
...
std::string spreadsheetPrinter(std::vector<uint8_t> m, uint32_t height, uint32_t width, std::vector<std::string> linesDescription) {
    if (linesDescription.size() != height) return "ERROR: height != linesDescription";

    uint8_t h_indicesSize = 1;
    for (uint32_t i = height; i >= 10; i /= 10) h_indicesSize++;
    uint8_t w_indicesSize = 1;
    for (uint32_t i = width; i >= 10; i /= 10) w_indicesSize++;
    ...
    for (size_t line = 0; line < height; line++) {
        ...
        for (size_t col = 0; col < width; col++) {
            ...
            if (m[line * width + col]) oss << WHITE_DESAT_BACKGROUND << BLACK_NORMAL_COLOR;
            if (col % 2) oss << BOLD;
        ...
        if (linesDescription[line].size()) oss << " -> " << linesDescription[line];
        oss << NORMAL;
    }
    return oss.str();
}